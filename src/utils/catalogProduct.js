...
export const normalizeVariant = (v) => {
  const effectiveModel =
    v?.effectivePreviewModelUrl ||
    v?.previewModelUrl ||
    v?.designTemplateFileUrl ||
    null;
  const effectiveThumb =
    v?.effectiveThumbnailUrl || v?.designTemplateThumbnailUrl || v?.thumbnailUrl || null;
  ...
  return {
    ...
    materialId: v?.materialId != null ? String(v.materialId) : v?.materialId,
